...
set _DeploySetParametersFile=%RootPath%Case3.PcSWinkelen.Host.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Case3.PcSWinkelen.Host.zip" (
echo "%RootPath%Case3.PcSWinkelen.Host.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Case3.PcSWinkelen.Host.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Case3.PcSWinkelen.Host.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%Case3.PcSWinkelen.Host.deploy-readme.txt" (
...
start notepad "%RootPath%Case3.PcSWinkelen.Host.deploy-readme.txt"
)
echo =========================================================
goto :eof
